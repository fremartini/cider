...
func Test_Contains(t *testing.T) {
	tests := map[string]struct {
		...
		"ip inside range":     {ip: "10.0.0.5", ipRange: cidr.NewBlock(ip.NewIp("10.0.0.0"), "28"), expected: true},
		"ip outside range":    {ip: "10.0.0.17", ipRange: cidr.NewBlock(ip.NewIp("10.0.0.0"), "28"), expected: false},
		"range inside range":  {ip: "10.0.0.4/30", ipRange: cidr.NewBlock(ip.NewIp("10.0.0.0"), "28"), expected: true},
		"range outside range": {ip: "10.0.0.16/29", ipRange: cidr.NewBlock(ip.NewIp("10.0.0.0"), "28"), expected: false},
	}
